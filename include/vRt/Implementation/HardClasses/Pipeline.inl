...
namespace _vt {
    ...
    inline VtResult createRayTracingPipeline(std::shared_ptr<Device> _vtDevice, const VtRayTracingPipelineCreateInfo& info, std::shared_ptr<Pipeline>& _vtPipeline) {
        ...
        if (info.resolveModule.module) vtPipeline->_resolvePipeline = createCompute(VkDevice(*_vtDevice), info.resolveModule, *vtPipeline->_pipelineLayout, VkPipelineCache(*_vtDevice));
