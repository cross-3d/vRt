...
namespace _vt {
    ...
    inline VtResult createBuffer(std::shared_ptr<Device> device, const VtDeviceBufferCreateInfo& cinfo, std::shared_ptr<RoledBuffer<U>>& _vtBuffer) {
        ...
        if constexpr (U == VMA_MEMORY_USAGE_CPU_TO_GPU) { usageFlag |= VkBufferUsageFlagBits::VK_BUFFER_USAGE_TRANSFER_SRC_BIT; }
        else // from src only
            if constexpr (U == VMA_MEMORY_USAGE_GPU_TO_CPU) { usageFlag |= VkBufferUsageFlagBits::VK_BUFFER_USAGE_TRANSFER_DST_BIT; }
            else // to dst only
            {
