...
layout ( binding = 2, set = RS_SET, align_ssbo ) coherent buffer VT_CLOSEST_HITS { uint closestHits[]; };
layout ( binding = 3, set = RS_SET, align_ssbo ) coherent buffer VT_MISS_HITS { uint missHits[]; };
...
layout ( binding = 6, set = RS_SET, align_ssbo ) readonly   buffer VT_CANVAS_INFO {
    int32_t currentGroup, maxRayCount, maxHitCount, closestHitOffset;
    i32vec2 size; int32_t lastIteration, iteration;
} stageUniform;
...
// TODO: add support of grouped regions (multiple-binding)
layout ( binding = 5 , set = RS_SET, align_ssbo ) coherent buffer VT_RAY_INDICES { uint rayGroupIndices[]; };
...
// TODO: add support of grouped regions (multiple-binding)
layout ( binding = 13, set = RS_SET, align_ssbo ) readonly coherent buffer VT_RAY_INDICES_READ { uint rayGroupIndicesRead[]; };
...
const uint R_BLOCK_WIDTH = 8, R_BLOCK_HEIGHT = 8, R_BLOCK_SIZE = R_BLOCK_WIDTH * R_BLOCK_HEIGHT;
...
int vtReuseRays(in highp u32vec2 c2d, in uint type, in lowp int rayID) {
    [[flatten]] if (max3_vec(f16_f32(ray.dcolor)) > 1e-3f) {
        vtParameteri(RAY_TYPE, ray.dcolor.y, int(type));
...
int vtEmitRays(in highp u32vec2 c2d, in uint type) { return vtReuseRays(c2d, type, -1); };
#endif
...
int vtRayIdx(in int lidx) { return int(lidx >= 0 && lidx < MAX_RAYS ? rayGroupIndicesRead[RPG_OFF + lidx] : 0)-1; };
...
int vtClosestId(in int id, in lowp int g) {return int(id >= 0 && id < MAX_HITS ? closestHits[(g+1)*(2*MAX_HITS) + HPG_OFF + id] : 0)-1; };
int vtMissId(in int id, in lowp int g) { return int(id >= 0 && id < MAX_HITS ? missHits[HPG_OFF + id] : 0)-1; };

int vtMakeAttribID(in int hAttribID, in int sub) { return int(HPG_OFF+(hAttribID-1))*ATTRIB_EXTENT + sub; };
